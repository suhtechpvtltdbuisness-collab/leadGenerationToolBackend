...
app.get("/api/search-hospitals", async (req, res) => {
  ...
  try {
        ...
        return items
          .map((item) => {
            ...
            const name =
              ...
              "No Name";
            const details =
              Array.from(item.querySelectorAll(".W4Efsd")).map(
                (d) => d.innerText,
              ) || [];
            const rating =
              item.querySelector(".MW4T7d")?.innerText || "No Rating";
            const address =
              details.find((d) => d.includes(",") || /\d+/.test(d)) ||
              "No Address";
            const phone =
              details.find(
                (d) => d.includes("+") || /^\d{5,}/.test(d.replace(/\s/g, "")),
              ) || "No Phone";
            const websiteLink =
              item.querySelector('a[data-value="Website"]') ||
              item.querySelector('a[aria-label*="Website"]') ||
              Array.from(item.querySelectorAll("a")).find(
                (a) => a.href && !a.href.includes("google.com/maps"),
              );
            return {
              name,
              rating,
              address,
              phone,
              website: websiteLink?.href || "No Website",
            };
          })
    ...
    while (allResults.length < maxResults + startFrom) {
      ...
      console.log(
        `Extracted: ${allResults.length} / Target: ${maxResults + startFrom}`,
      );
  ...
  try {
    ...
    if (!process.env.VERCEL) {
      ...
      const server = app.listen(PORT, "0.0.0.0", () => {
        ...
        console.log(
          `📡 Registered Routes: GET /health, POST /api/leads, GET /api/search-hospitals`,
        );
      });
      ...
      console.log(
        "🔄 Running in serverless mode (Vercel) - DB connection will be lazy-loaded per request",
      );
    }
  } catch (error) {
    console.error(
